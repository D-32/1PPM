...
class KMeans<Label: Hashable> {
  let numCenters: Int
  let labels: [Label]
  fileprivate(set) var centroids = [Vector]()
  var __classifications = [[Vector]]()
  ...
  fileprivate func indexOfNearestCenter(_ x: Vector, centers: [Vector]) -> Int {
  ...
  func trainCenters(_ points: [Vector], convergeDistance: Double) {
    ...
    repeat {
      ...
      //print("centers with values: ",newCenters.filter({$0.data.reduce(0, +) > 0}).count)
      ...
      //print("!! centerMoveDist", centerMoveDist)

      centers = newCenters
    } while centerMoveDist > convergeDistance

    __classifications = .init(repeating: [], count: numCenters)
    for p in points {
      let classIndex = indexOfNearestCenter(p, centers: centers)
      __classifications[classIndex].append(p)
    }
